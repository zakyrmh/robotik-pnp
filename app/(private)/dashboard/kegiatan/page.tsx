...
import { getAllEventsForCaang } from "@/app/actions/or-events.action";
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Seluruh jadwal pelatihan, wawancara, dan kegiatan Open Recruitment
            UKM Robotik PNP.
          </p>
        </div>
      </div>
...
async function KegiatanList() {
  const { data: eventsRaw, error } = await getAllEventsForCaang();
    ...
    return (
      <div className="rounded-xl border border-dashed p-12 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1 text-balance max-w-xs mx-auto">
          Panitia belum mempublikasikan jadwal kegiatan apa pun. Silakan cek
          berkala atau hubungi admin di grup komunitas.
        </p>
      </div>
    );
  }
...
function EventCard({ event }: { event: OrEvent }) {
  const isPast =
    new Date(`${event.event_date}T${event.start_time}`) < new Date();

  return (
    <div
      className={`group relative flex flex-col rounded-xl border bg-card transition-all hover:shadow-md ${isPast ? "opacity-80" : ""}`}
    >
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          {event.status === "completed" ? (
            <Badge
              variant="outline"
              className="text-[10px] h-5 bg-emerald-500/15 text-emerald-600 border-emerald-500/25"
            >
              Selesai
            </Badge>
          ) : isPast ? (
            ...
            <Badge
              variant="outline"
              className="text-[10px] h-5 border-blue-500/50 text-blue-600 animate-pulse"
            >
              Akan Datang
            </Badge>
          )}
