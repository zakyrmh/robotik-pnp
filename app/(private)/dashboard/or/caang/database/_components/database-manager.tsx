...
const STATUS_COLORS: Record<OrRegistrationStatus, string> = {
  ...
  graduated:
    "bg-emerald-600/20 text-emerald-700 border-emerald-600/30 font-bold",
};
...
export function DatabaseManager({ initialRegistrations }: Props) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      ...
      if (profileRes.error) {
        showFeedback("error", profileRes.error);
        return;
      }
      ...
      if (regRes.error) {
        showFeedback("error", regRes.error);
        return;
      }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {[
          ...
          <div
            key={s.label}
            className="rounded-lg border bg-card p-3 shadow-sm text-center"
          >
                  ...
                  <td className="px-3 py-2.5 text-center">
                    ...
                    >
                      {r.current_step === "completed"
                        ? "✅"
                        : r.current_step === "payment"
                          ? "💳"
                          : r.current_step === "documents"
                            ? "📄"
                            : "📝"}
                    </Badge>
                  </td>
      ...
      {editReg && (
        ...
        >
          <div
            className="bg-card rounded-xl shadow-lg w-full max-w-xl max-h-[85vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-card border-b px-5 py-3 flex items-center justify-between z-10">
              <p className="font-semibold">
                Edit Data — {editReg.full_name}
              </p>
              <button
                type="button"
                onClick={() => setEditReg(null)}
            ...
            <div className="px-5 py-4 space-y-3">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    value={editYearEnrolled}
                    onChange={(e) => setEditYearEnrolled(e.target.value)}
                  />
                </div>
                <div className="space-y-1.5 sm:col-span-2">
                  <Label className="text-xs flex items-center gap-1">
                    <MapPin className="size-3" /> Alamat Domisili
                  </Label>
                  <Input
                    value={editAddress}
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <Textarea
                    value={editOrgExp}
                    onChange={(e) => setEditOrgExp(e.target.value)}
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`cursor-pointer text-[10px] px-2 py-1 rounded-md border flex items-center gap-1 ${
                        doc.url
                          ? "hover:bg-accent text-blue-600"
                          : "text-muted-foreground opacity-50 cursor-not-allowed"
                      }`}
                    >
                      <ImageIcon className="size-3" /> {doc.label}
                    </button>
                  ))}
                </div>
              </div>
