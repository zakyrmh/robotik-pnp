...
/**
 * JadwalManager — CRUD jadwal kegiatan Open Recruitment.
 *
 * Fitur:
 * - Daftar kegiatan dalam grid card
 * - Tambah / edit kegiatan (dialog)
 * - Hapus kegiatan
 * - Konfigurasi absensi & poin
 * - Filter search
 */
...
export function JadwalManager({ initialEvents }: Props) {
  const [isPending, startTransition] = useTransition();
  const [events, setEvents] = useState<OrEvent[]>(initialEvents);
  const [searchTerm, setSearchTerm] = useState("");

  // UI State
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingEvent, setEditingEvent] = useState<Partial<OrEvent> | null>(
    null,
  );
  ...
  const handleSave = () => {
    startTransition(async () => {
      const result = await adminSaveEvent(formData);
      if (!result.error) {
        showFeedback(
          "success",
          editingEvent
        ...
        showFeedback("error", result.error || "Gagal menyimpan kegiatan.");
      }
    });
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!result.error) {
      ...
      showFeedback("error", result.error || "Gagal menghapus kegiatan.");
    }
  };
      ...
      {filteredEvents.length === 0 ? (
        <div className="rounded-xl border border-dashed bg-card p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">
            Klik tombol di atas untuk menambahkan kegiatan baru.
          </p>
        </div>
      ) : (
          ...
          {filteredEvents.map((event) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2 px-4 pt-3 pb-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button
                      type="button"
                      className="cursor-pointer p-1 hover:bg-accent rounded shrink-0"
                    >
              ...
              <div className="px-4 pb-3 space-y-1.5">
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    ...
                    {event.end_time?.substring(0, 5) || "Selesai"}
                  </span>
                </div>
                {event.location && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <div className="border-t px-4 py-2 flex items-center justify-between">
                ...
                >
                  {event.allow_attendance ? "✓ Absensi Aktif" : "Tanpa Absensi"}
                </span>
        ...
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto rounded-xl shadow-lg border">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs font-medium">
              Isi detail kegiatan untuk informasikan ke caang.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setFormData({
                      ...formData,
                      execution_mode: val as OrEventMode,
                    })
                  }
                >
                  <SelectTrigger className="h-9 text-xs">
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Catat kehadiran caang dan perolehan poin.
                  </p>
                </div>
                <Switch
                  checked={formData.allow_attendance}
                  onCheckedChange={(val) =>
                    setFormData({ ...formData, allow_attendance: val })
                  }
                />
              </div>
                ...
                <div className="grid gap-4 sm:grid-cols-3 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="number"
                      className="h-8 text-xs"
                      value={formData.points_present}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          points_present: parseInt(e.target.value) || 0,
                        })
                      }
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          points_late: parseInt(e.target.value) || 0,
                        })
                      }
                    />
                  </div>
