 ...
 */
function convertDocToRegistration(
  docId: string,
 ...
 */
export async function getRegistrationsByOrPeriod(
  orPeriod: string
 ...
 */
export async function submitRegistration(id: string): Promise<void> {
  try {
    const docRef = doc(db, COLLECTION_NAME, id);
    await updateDoc(docRef, {
...
// Alias for backward compatibility with registration-service.ts
export const getRegistration = getRegistrationById;
