...
describe("Symmetric Crypto Helper", () => {
  ...
  it("should encrypt and decrypt compact attendance QR payload correctly", () => {
    const payload = {
      profile_id: "12345678-1234-1234-1234-123456789abc",
      activity_id: "87654321-4321-4321-4321-cba987654321",
      generated_at: 1754300000000,
      coordinates: { latitude: -0.95, longitude: 100.35 },
    };
    const token = encryptToken(payload);
    expect(token).toBeTypeOf("string");

    const decrypted = decryptToken(token);
    expect(decrypted.profile_id).toBe(payload.profile_id);
    expect(decrypted.activity_id).toBe(payload.activity_id);
    expect(decrypted.generated_at).toBe(payload.generated_at);
    expect(decrypted.coordinates?.latitude).toBeCloseTo(-0.95, 2);
    expect(decrypted.coordinates?.longitude).toBeCloseTo(100.35, 2);
  });

  it("should handle compact attendance QR payload when writeBigInt64BE is missing (browser environment fallback)", () => {
    const origWrite = Buffer.prototype.writeBigInt64BE;
    const origRead = Buffer.prototype.readBigInt64BE;
    try {
      // Simulating browser polyfill Buffer where BigInt64 methods are missing
      Buffer.prototype.writeBigInt64BE = undefined;
      Buffer.prototype.readBigInt64BE = undefined;
      ...
      Buffer.prototype.writeBigInt64BE = origWrite;
      Buffer.prototype.readBigInt64BE = origRead;
    }
  });
...
describe("Attendance Server Actions", () => {
  ...
  describe("generateAttendanceQR", () => {
    ...
    it("should reject if activity time window has not started yet", async () => {
      ...
      mockSupabase.single.mockResolvedValueOnce({
        data: { start_date: futureStart, end_date: futureEnd },
      });
    ...
    it("should reject if activity time window has already ended", async () => {
      ...
      mockSupabase.single.mockResolvedValueOnce({
        data: { start_date: pastStart, end_date: pastEnd },
      });
    ...
    it("should succeed and return encrypted token if user is authorized and time window is open", async () => {
      ...
      mockSupabase.single.mockResolvedValueOnce({
        data: { start_date: pastStart, end_date: futureEnd },
      });
  ...
  describe("scanAttendanceQR", () => {
    ...
    it("should reject if the QR token is expired", async () => {
      ...
      const oldPayload = {
        ...
        generated_at: Date.now() - 10 * 60 * 1000,
      };
  ...
  describe("manualOverrideAttendance", () => {
    it("should reject if admin notes are empty", async () => {
      ...
      const res = await manualOverrideAttendance(
        "attendance-id",
        "hadir",
        "  ",
      );
