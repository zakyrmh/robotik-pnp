...
export function QrisPaymentView({
  ...
  const availableBankAccounts: BankAccount[] = (() => {
    if (
      eventSettings?.bank_accounts &&
      eventSettings.bank_accounts.length > 0
    ) {
  ...
  const handleSubmitProof = async (e: React.FormEvent) => {
    ...
    if (!proofUrl) {
      setUploadProofError(
        "Silakan upload gambar bukti transfer terlebih dahulu.",
      );
      return;
    }
    ...
    return (
      <div className="bg-card p-8 rounded-lg border border-border shadow-soft text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Pembayaran pendaftaran tim <strong>{reg.team_name}</strong> telah
        ...
        {reg.category?.whatsapp_group_url && (
          <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg max-w-md mx-auto space-y-2">
            <span className="text-xs font-bold text-emerald-800 flex items-center justify-center gap-1.5">
              <MessageSquare className="w-4 h-4 text-emerald-600" /> Grup
              WhatsApp Official {reg.category.name}
            </span>
            <p className="text-xs text-emerald-700">
              Silakan bergabung ke grup WhatsApp untuk mendapatkan informasi
              teknis perlombaan:
            </p>
            <a
      ...
      <div className="bg-card rounded-xl border border-border shadow-soft p-6 sm:p-8 space-y-6">
        ...
        {isPendingVerification && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-center space-y-2">
            ...
            <p className="text-xs text-amber-700 max-w-md mx-auto">
              Bukti transfer bank telah berhasil dikirim. Panitia sedang
              melakukan pengecekan mutasi rekening. Anda akan menerima email
              begitu pembayaran disetujui.
            </p>
          </div>
        )}
        ...
        {isRejected && (
          <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl text-center space-y-2">
            ...
            <p className="text-xs text-rose-700 max-w-md mx-auto">
              Silakan lakukan transfer sesuai nominal dan unggah ulang gambar
              bukti transfer yang benar di bawah ini.
            </p>
          </div>
        )}
        ...
        <div className="bg-muted/40 border border-border rounded-xl p-5 space-y-3">
          <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
            <Building2 className="w-4 h-4 text-primary" /> Opsi Rekening Bank
            Tujuan Transfer
          </h3>
          <p className="text-xs text-muted-foreground">
            Anda dapat mentransfer biaya pendaftaran ke salah satu rekening bank
            berikut:
          </p>
                  ...
                  {isUploadingProof ? (
                    ...
                    <>
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        Pastikan tanggal, nominal, & nama pengirim terlihat
                        jelas.
                      </span>
                    </>
                  )}
