...
export function ManualPaymentVerificationList({
  ...
  // Modal Reject State
  const [rejectModalReg, setRejectModalReg] = useState<EventRegistration | null>(
    null,
  );
  ...
  const handleApprove = async (reg: EventRegistration) => {
    ...
    if (res.success) {
      setRegistrations(
        registrations.map((r) =>
          r.id === reg.id
            ? { ...r, payment_status: "paid", paid_at: new Date().toISOString() }
            : r,
        ),
      );
    } else {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Terdapat <strong className="text-amber-700 font-bold">{pendingCount}</strong> bukti pembayaran yang perlu diverifikasi admin.
            </p>
          </div>
        </div>
                  ...
                  <div className="flex items-start justify-between gap-2 border-b border-slate-100 pb-3">
                    ...
                    <span
                      className={cn(
                        ...
                        !isPending && !isApproved && !isRejected && "bg-slate-100 text-slate-600 border-slate-200",
                      )}
                    >
                      {isPending && "Menunggu Verifikasi"}
                      {isApproved && "Lunas / Disetujui"}
                      {isRejected && "Ditolak"}
                      {!isPending && !isApproved && !isRejected && reg.payment_status}
                    </span>
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 max-w-md w-full p-6 space-y-4 shadow-xl">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Tim: <strong>{rejectModalReg.team_name}</strong> ({rejectModalReg.registration_code})
              </p>
            </div>
            ...
            <form onSubmit={handleConfirmReject} className="space-y-4">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Alasan ini akan dikirimkan ke email peserta agar peserta dapat mengunggah ulang bukti transfer yang benar.
                </p>
              </div>
