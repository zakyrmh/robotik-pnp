...
export function PresensiClient({
  ...
  const isManager =
    ...
    (userRole === "admin-or" && activeAudience === "caang");

  const [activeTab, setActiveTab] = useState<
    "members" | "activities" | "personal"
  >(isManager ? "members" : "personal");
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto pb-12">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 shadow-xs">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-[10px] uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-[#1e3a8a]/10 text-[#1e3a8a] dark:bg-blue-900/40 dark:text-blue-300 font-bold">
                {isManager ? "MANAJEMEN REKAP PRESENSI" : "RIWAYAT KEHADIRAN"}
              </span>
            ...
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-[#0a192f] dark:text-slate-50 tracking-tight">
              {isManager
                ? `Manajemen & Rekap Presensi — ${activeAudience === "caang" ? "Caang" : "Anggota"}`
                : "Presensi Kehadiran Saya"}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">
              {isManager
                ? `Monitoring rekapitulasi presensi ${activeAudience === "caang" ? "calon anggota (caang)" : "anggota aktif UKM Robotik"}, analisis kedisiplinan per kegiatan, dan akumulasi kehadiran.`
                : "Daftar riwayat kehadiran dan catatan presensi Anda pada seluruh agenda kegiatan UKM Robotik PNP."}
            </p>
          </div>
        </div>

        {/* Audience Switcher Header Tabs */}
        {canSwitchAudience && (
          <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center gap-2">
            <button
              type="button"
              onClick={() => {
                setActiveAudience("caang");
                if (userRole === "admin-or") setActiveTab("members");
              }}
              className={`px-4 py-2 rounded-xl font-mono text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
            ...
            <button
              type="button"
              onClick={() => {
                setActiveAudience("anggota");
                if (userRole === "admin-or") setActiveTab("personal");
              }}
              className={`px-4 py-2 rounded-xl font-mono text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
        ...
        {isManager && (
          <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center gap-2 overflow-x-auto no-scrollbar">
            <button
              type="button"
