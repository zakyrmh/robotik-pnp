...
export function ActivityDetailClient({
  ...
  const getActivityStatusBadge = (activity: ActivityItem) => {
    ...
    switch (status) {
      ...
      case "ongoing":
        return (
          <Badge className="bg-[#1c69d4] hover:bg-[#1c69d4]/90 font-mono text-[10px] uppercase tracking-wider rounded-none">
            Berlangsung
          </Badge>
        );
      case "completed":
        return (
          <Badge
            variant="outline"
            className="font-mono text-[10px] uppercase tracking-wider text-zinc-500 rounded-none border-zinc-200"
          >
            Selesai
          </Badge>
        );
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0a0f24] text-zinc-900 dark:text-zinc-50 font-sans selection:bg-[#1c69d4] selection:text-white pb-24">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {(["hadir", "izin", "sakit", "alfa", "telat"] as StatusAbsensi[]).map(
            (s) => (
              <div
                key={s}
                className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-none p-4 flex flex-col items-center justify-center"
              >
                <span
                  className={`text-3xl font-bold ${STATUS_CONFIG[s].color}`}
                >
                  {aggregatedStats[s]}
                </span>
                <span className="font-mono text-xs uppercase tracking-wider text-zinc-500 mt-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
                <tr>
                  <th className="p-4 font-mono text-[10px] uppercase tracking-wider text-zinc-500 w-[60px] text-center">
                    No
                  </th>
                  <th className="p-4 font-mono text-[10px] uppercase tracking-wider text-zinc-500">
                    Caang
                  </th>
                  <th className="p-4 font-mono text-[10px] uppercase tracking-wider text-zinc-500 w-[200px] text-center">
                    Status Kehadiran
                  </th>
                </tr>
              </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4 text-center relative">
                          <button
                            onClick={() =>
                              setOverrideCell(isOpen ? null : item.profileId)
                            }
                            disabled={isPending}
                            ...
                            <div className="absolute z-50 top-full left-1/2 -translate-x-1/2 mt-1 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 shadow-lg rounded-none min-w-[120px]">
                              {(
                                ...
                                <button
                                  key={s}
                                  onClick={() =>
                                    handleAttendanceChange(item.profileId, s)
                                  }
