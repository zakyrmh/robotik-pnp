...
let mockGetStudyProgramsResult: any = [
  { id: "sp-1", name: "Teknik Informatika", degree: "D4" },
];
...
describe("Step 2 — StepPersonal: Validasi Field Wajib", () => {
  ...
  it('[S2-TC2] Menampilkan toast.error jika "Nama Panggilan" kosong', () => {
    renderStep2();
    fireEvent.change(screen.getByTestId("John Doe"), {
      target: { value: "Zaky Ramadhani" },
    });
    fireEvent.click(screen.getByRole("button", { name: /Lanjut/i }));
...
describe("Step 3 — StepAcademic: Validasi & Dropdown Jurusan/Prodi", () => {
  ...
  it('[S3-TC1] Menampilkan toast.error jika "Asal Sekolah" kosong saat klik Lanjut', async () => {
    render(
      <StepAcademic onNext={vi.fn()} onPrev={vi.fn()} initialData={null} />,
    );
  ...
  it("[S3-TC2] Menampilkan toast.error jika Jurusan belum dipilih", async () => {
    ...
    await waitFor(() => {});
    fireEvent.change(screen.getByTestId("SMKN 1 Padang"), {
      target: { value: "SMA 1 Padang" },
    });
    fireEvent.click(screen.getByRole("button", { name: /Lanjut/i }));
  ...
  it("[S3-TC3] Menampilkan toast.error jika Program Studi belum dipilih", async () => {
    ...
    await waitFor(() => expect(lanjutBtn).not.toBeDisabled(), {
      timeout: 5000,
    });
...
describe("Step 4 — StepCommitment: Validasi Motivasi & Upload Bukti Medsos", () => {
  ...
  it('[S4-TC1] Menampilkan toast.error jika field "Motivasi" kosong saat klik Lanjut', () => {
    render(
      <StepCommitment onNext={vi.fn()} onPrev={vi.fn()} initialData={null} />,
    );
    fireEvent.click(
  ...
  it("[S4-TC2] Menampilkan 3 komponen UploadTile untuk bukti media sosial", () => {
    ...
    expect(
      screen.getByTestId("upload-tile-Follow Instagram Robotik"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("upload-tile-Follow Instagram MRC"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("upload-tile-Subscribe YT Robotik"),
    ).toBeInTheDocument();
  });
  ...
  it("[S4-TC3] Jika motivasi terisi, saveCommitmentData dipanggil dan onNext terpicu", async () => {
    ...
    fireEvent.change(screen.getByTestId("Apa alasan Anda ingin bergabung?"), {
...
describe("Step 5 — StepUpload: Validasi File & Submisi Akhir", () => {
  ...
  async function triggerPasFotoViaCrop(
    pasFotoInput: HTMLElement,
  ): Promise<void> {
    ...
    const croppedFile = new File(["cropped"], "pas_cropped.jpg", {
      type: "image/jpeg",
    });
    await act(async () => {
  ...
  it("[S5-TC3] Menampilkan toast.error jika Metode Pembayaran belum dipilih", async () => {
    ...
    const mockPayment = new File(["pdf"], "bukti.pdf", {
...
describe("Integrasi — OnboardingClient: Alur Stepper & Logika Legacy Member", () => {
  ...
  it("[INT-TC4] Skenario NIM gagal: checkLegacyMember returns success:false → toast.error", async () => {
    ...
    await act(async () => {
      fireEvent.click(
        screen.getByRole("button", { name: /Cek Validasi NIM/i }),
      );
    });

    await waitFor(() => {
      expect(mockToastError).toHaveBeenCalledWith(
        "Gagal memeriksa NIM. Silakan coba lagi.",
      );
    });
    // Tetap di step 1
    expect(screen.getByText("Validasi Identitas")).toBeInTheDocument();
  ...
  it("[INT-TC6] Skenario NIM gagal karena pendaftaran ditutup: checkLegacyMember returns success:false, isClosed:true → closedError ditunjukkan di UI", async () => {
    mockCheckLegacyResult = {
      success: false,
      isClosed: true,
      error: "Pendaftaran ditutup. Silakan tunggu pendaftaran selanjutnya.",
    };
    render(<OnboardingClient initialProgress={defaultProgress} />);
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          "Pendaftaran ditutup. Silakan tunggu pendaftaran selanjutnya.",
        ),
      ).toBeInTheDocument();
    });
    // Tetap di step 1
