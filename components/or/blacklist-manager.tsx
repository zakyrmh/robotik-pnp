...
import { addToBlacklist, removeFromBlacklist, getBlacklist } from '@/app/actions/or.action'
...
export function BlacklistManager({ initialBlacklist, members }: Props) {
  ...
  const handleAdd = () => {
    ...
    if (formIsPermanent === 'false' && !formExpiresAt) {
      showFeedback('error', 'Tanggal kedaluwarsa wajib diisi untuk blacklist sementara.')
      return
    }
    startTransition(async () => {
      const result = await addToBlacklist({
        ...
        expiresAt: formIsPermanent === 'false' ? new Date(formExpiresAt).toISOString() : undefined,
      })
  ...
  const availableMembers = members.filter((m) => !blacklistedUserIds.has(m.id))
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <div className="rounded-lg border bg-card px-3 py-2 shadow-sm">
          ...
          <p className="text-lg font-bold tabular-nums text-red-600">{blacklist.length}</p>
        </div>
        <div className="rounded-lg border bg-card px-3 py-2 shadow-sm">
          <p className="text-[10px] text-muted-foreground">Permanen</p>
          <p className="text-lg font-bold tabular-nums">{blacklist.filter((b) => b.is_permanent).length}</p>
        </div>
        <div className="rounded-lg border bg-card px-3 py-2 shadow-sm">
      ...
      {/* Tombol */}
      <Button onClick={() => { setShowForm(!showForm); resetForm() }} className="cursor-pointer bg-red-600 hover:bg-red-700">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Select value={formUserId} onValueChange={setFormUserId}>
                <SelectTrigger><SelectValue placeholder="Pilih caang..." /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Textarea value={formReason} onChange={(e) => setFormReason(e.target.value)} placeholder="Alasan blacklist..." className="min-h-[60px]" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">URL Bukti (opsional)</Label>
              <Input value={formEvidenceUrl} onChange={(e) => setFormEvidenceUrl(e.target.value)} placeholder="https://..." />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Tipe</Label>
              <Select value={formIsPermanent} onValueChange={setFormIsPermanent}>
            ...
            {formIsPermanent === 'false' && (
              <div className="space-y-1.5">
                <Label className="text-xs flex items-center gap-1"><CalendarDays className="size-3" /> Kedaluwarsa *</Label>
                <Input type="date" value={formExpiresAt} onChange={(e) => setFormExpiresAt(e.target.value)} />
              </div>
            )}
          </div>
          <div className="flex gap-2">
            <Button onClick={handleAdd} disabled={isPending} className="cursor-pointer bg-red-600 hover:bg-red-700">
      ...
      {blacklist.length === 0 ? (
        <div className="rounded-xl border bg-card p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">Belum ada caang yang diblacklist.</p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-4 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge variant="outline" className={`text-[10px] ${b.is_permanent ? 'bg-red-500/15 text-red-600 border-red-500/25' : 'bg-amber-500/15 text-amber-600 border-amber-500/25'}`}>
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-muted-foreground mt-0.5 flex-wrap">
                    <span>Ditambahkan: {new Date(b.created_at).toLocaleDateString('id-ID')}</span>
                    {!b.is_permanent && b.expires_at && (
                      <span className="flex items-center gap-0.5"><Clock className="size-2.5" /> Exp: {new Date(b.expires_at).toLocaleDateString('id-ID')}</span>
                    )}
                  </div>
                </div>
                <div className="flex gap-1.5 shrink-0">
                  {b.evidence_url && (
                    <Button size="sm" variant="outline" className="text-xs cursor-pointer" onClick={() => setPreviewUrl(b.evidence_url)}>
                      <Image className="size-3" /> Bukti
                    </Button>
                  )}
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button size="sm" variant="outline" className="text-xs cursor-pointer border-emerald-500/30 text-emerald-600 hover:bg-emerald-500/10">
                        <Trash2 className="size-3" /> Hapus
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Hapus dari Blacklist?</AlertDialogTitle>
                        <AlertDialogDescription>
                      ...
                      <AlertDialogFooter>
                        ...
                        <Button onClick={() => handleRemove(b.id)} disabled={isPending} className="cursor-pointer bg-emerald-600 hover:bg-emerald-700">Hapus dari Blacklist</Button>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
      ...
      {previewUrl && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center cursor-pointer" onClick={() => setPreviewUrl(null)}>
          <div className="relative max-w-3xl max-h-[80vh]">
            ...
            <img src={previewUrl} alt="Bukti blacklist" className="rounded-lg max-h-[80vh] object-contain" />
...
export function BlacklistSkeleton() {
  return (
    <div className="space-y-4">
      <div className="flex gap-2">{[1, 2, 3].map((i) => <Skeleton key={i} className="h-16 w-24 rounded-lg" />)}</div>
