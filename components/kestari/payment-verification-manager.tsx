...
import { getPiketFines, verifyFinePayment } from "@/app/actions/kestari.action";
import type {
  PiketPeriod,
  PiketFineWithUser,
  PiketFineStatus,
  ...
  const handleVerify = (fineId: string, status: "paid" | "waived") => {
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        showFeedback(
          "success",
          status === "paid" ? "Pembayaran diverifikasi." : "Denda dibebaskan.",
        );
        reload();
      }
    });
      ...
      {displayFines.length === 0 ? (
        <div className="rounded-xl border bg-card p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">
            Tidak ada denda dengan filter yang dipilih.
          </p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-4 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge
                      variant="outline"
                      className={`text-[10px] ${STATUS_COLORS[f.status]}`}
                    >
                ...
                {f.payment_proof_url && (
                  ...
                  >
                    ...
                    <Image className="size-3" aria-hidden="true" /> Bukti Bayar
                  </button>
                )}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Denda Rp {f.amount.toLocaleString("id-ID")} untuk{" "}
                            {f.full_name} di bulan {f.month_year} akan
                            dibebaskan (dispensasi).
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Batal</AlertDialogCancel>
                          <Button
                            onClick={() => handleVerify(f.id, "waived")}
                            disabled={isPending}
                            className="cursor-pointer"
                          >
                            {isPending ? (
