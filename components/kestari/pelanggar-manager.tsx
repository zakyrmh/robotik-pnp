...
const STATUS_ICONS: Record<PiketFineStatus, React.ComponentType<{ className?: string }>> = {
  unpaid: Banknote,
...
export function PelanggarManager({ period, initialFines }: Props) {
  ...
  const handleGenerate = () => {
    startTransition(async () => {
      const result = await generateFinesForMonth(period.id, generateMonth)
      if (result.error) {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-4 shadow-sm space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Label className="text-sm font-semibold">Generate Denda Otomatis</Label>
        </div>
        <p className="text-xs text-muted-foreground">
          Buat denda untuk anggota yang belum piket di bulan tertentu.
          Anggota yang sudah approve piketnya tidak akan terkena denda.
        </p>
        <div className="flex flex-wrap items-end gap-2">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Generate Denda untuk {generateMonth}?</AlertDialogTitle>
                <AlertDialogDescription>
                  Denda Rp {period.fine_amount.toLocaleString('id-ID')} akan dibuat untuk anggota yang
                  belum submit atau piketnya ditolak di bulan {generateMonth}.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Batal</AlertDialogCancel>
                <Button onClick={handleGenerate} disabled={isPending} className="cursor-pointer bg-orange-600 hover:bg-orange-700">
      ...
      {totalUnpaid > 0 && (
        <div className="rounded-lg border border-orange-500/20 bg-orange-500/5 px-4 py-3 text-sm text-orange-700 dark:text-orange-400">
          <strong>{totalUnpaid}</strong> pelanggar belum bayar &middot; Total: <strong>Rp {totalAmount.toLocaleString('id-ID')}</strong>
        </div>
      )}
      ...
      {fines.length === 0 ? (
        <div className="rounded-xl border bg-card p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">Semua anggota sudah melaksanakan piket 🎉</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/30">
                ...
                <th className="px-3 py-2.5 text-center font-medium text-xs">Bulan</th>
                <th className="px-3 py-2.5 text-center font-medium text-xs">Denda</th>
                <th className="px-3 py-2.5 text-center font-medium text-xs">Status</th>
                <th className="px-3 py-2.5 text-left font-medium text-xs">Alasan</th>
              </tr>
            </thead>
            <tbody>
              {fines.map((f) => {
                ...
                return (
                  <tr key={f.id} className="border-b last:border-0 hover:bg-accent/30 transition-colors">
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="size-7 rounded-full bg-accent flex items-center justify-center text-[10px] font-bold shrink-0">
                          {(f.full_name || '?').charAt(0).toUpperCase()}
                        </div>
                        <span className="font-medium truncate max-w-[160px]">{f.full_name}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2.5 text-center text-xs text-muted-foreground">{f.month_year}</td>
                    <td className="px-3 py-2.5 text-center text-xs font-medium">
                      Rp {f.amount.toLocaleString('id-ID')}
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <Badge variant="outline" className={`text-[10px] gap-1 ${STATUS_COLORS[f.status]}`}>
                        <StatusIcon className="size-3" />
                        {PIKET_FINE_STATUS_LABELS[f.status]}
                      </Badge>
                    </td>
                    <td className="px-3 py-2.5 text-xs text-muted-foreground truncate max-w-[200px]">{f.reason}</td>
                  </tr>
                )
              })}
            </tbody>
