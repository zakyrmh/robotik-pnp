...
import {
  PIKET_SUBMISSION_STATUS_LABELS,
  WEEK_LABELS,
} from "@/lib/db/schema/kestari";
...
export function PiketVerificationManager({
  period,
  initialSubmissions,
}: Props) {
  ...
  const reload = () => {
    startTransition(async () => {
      const filters: { status?: PiketSubmissionStatus; monthYear?: string } =
        {};
      if (filterStatus !== "all")
        filters.status = filterStatus as PiketSubmissionStatus;
  ...
  const handleVerify = (
    ...
    startTransition(async () => {
      const result = await verifyPiketSubmission({ submissionId: id, status, notes: reason });
      if (result.error) {
        showFeedback("error", result.error);
      } else {
        showFeedback(
          "success",
          `Bukti piket ${status === "approved" ? "disetujui" : "ditolak"}.`,
        );
        setRejectReason("");
        reload();
      }
      ...
      {submissions.length === 0 ? (
        <div className="rounded-xl border bg-card p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">
            Belum ada anggota yang mensubmit bukti piket.
          </p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-4 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge
                      variant="outline"
                      className={`text-[10px] ${STATUS_COLORS[sub.status]}`}
                    >
                      {PIKET_SUBMISSION_STATUS_LABELS[sub.status]}
                    </Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {WEEK_LABELS[sub.assigned_week]} &middot; Piket:{" "}
                    {sub.piket_date} &middot; Bulan: {sub.month_year}
                  </p>
                  {sub.notes && (
                ...
                <div className="flex gap-1.5 shrink-0">
                  {sub.photo_before_url && (
                    ...
                    >
                      ...
                      <Image className="size-3" aria-hidden="true" /> Foto Sebelum
                    </button>
                  )}
                  {sub.photo_after_url && (
                    <button
                    ...
                    >
                      {/* eslint-disable-next-line jsx-a11y/alt-text */}
                      <Image className="size-3" aria-hidden="true" /> Foto Sesudah
                    </button>
                  )}
                </div>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Berikan alasan penolakan agar anggota bisa
                            memperbaiki.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <Textarea
                        ...
                        <AlertDialogFooter>
                          ...
                          <Button
                            variant="destructive"
                            onClick={() =>
                              handleVerify(sub.id, "rejected", rejectReason)
                            }
                            disabled={isPending}
                            className="cursor-pointer"
                          >
