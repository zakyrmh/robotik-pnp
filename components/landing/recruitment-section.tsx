...
export function RecruitmentSection({
  ...
  function calculateTimeLeft(target: Date | null, current: Date | null): TimeLeft {
    if (!target || !current) return { days: 0, hours: 0, minutes: 0, seconds: 0 };
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
            ...
            <div className="flex-1 space-y-4 text-center md:text-left">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-primary/10 text-primary border border-primary/20">
                ...
                <span>{badgeText} {periodeRecruitment ? `(${periodeRecruitment})` : ""}</span>
              </div>

              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-foreground font-display">
                Open Recruitment <span className="text-primary">UKM Robotik PNP</span>
              </h2>

              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed max-w-xl">
                Bergabunglah bersama keluarga besar UKM Robotik Politeknik Negeri Padang. Asah skill hardware, software, mekanik, serta kompetisi tingkat nasional KRI.
              </p>
                ...
                <div className="inline-flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs sm:text-sm text-muted-foreground pt-2">
                  {startDate && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>Buka: <strong className="text-foreground font-medium">{formatDateStr(startDate)}</strong></span>
                    </div>
                  )}
                  {endDate && (
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-4 h-4 text-accent" />
                      <span>Tutup: <strong className="text-foreground font-medium">{formatDateStr(endDate)}</strong></span>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-3 pt-4">
                ...
                >
                  <Link href={state === "open" ? "/register" : "/pendaftaran-caang"}>
                    <UserPlus className="w-4 h-4" />
                    {state === "open" ? "Daftar Sekarang" : "Informasi Pendaftaran"}
                  </Link>
                </Button>
              ...
              {(state === "opening_soon" || state === "open") && (
                <div className="grid grid-cols-4 gap-2 sm:gap-3 w-full">
                  <div className="flex flex-col items-center justify-center bg-card border border-border p-2.5 sm:p-3 rounded-lg shadow-xs">
                    ...
                    <span className="text-[10px] sm:text-xs text-muted-foreground uppercase mt-1 font-medium">Hari</span>
                  </div>
                  <div className="flex flex-col items-center justify-center bg-card border border-border p-2.5 sm:p-3 rounded-lg shadow-xs">
                    <span className="text-2xl sm:text-3xl font-bold text-foreground font-mono">
                      {String(timeLeft.hours).padStart(2, "0")}
                    </span>
                    <span className="text-[10px] sm:text-xs text-muted-foreground uppercase mt-1 font-medium">Jam</span>
                  </div>
                  <div className="flex flex-col items-center justify-center bg-card border border-border p-2.5 sm:p-3 rounded-lg shadow-xs">
                    <span className="text-2xl sm:text-3xl font-bold text-foreground font-mono">
                      {String(timeLeft.minutes).padStart(2, "0")}
                    </span>
                    <span className="text-[10px] sm:text-xs text-muted-foreground uppercase mt-1 font-medium">Menit</span>
                  </div>
                  <div className="flex flex-col items-center justify-center bg-card border border-border p-2.5 sm:p-3 rounded-lg shadow-xs">
                    <span className="text-2xl sm:text-3xl font-bold text-primary font-mono animate-pulse">
                      {String(timeLeft.seconds).padStart(2, "0")}
                    </span>
                    <span className="text-[10px] sm:text-xs text-muted-foreground uppercase mt-1 font-medium">Detik</span>
                  </div>
                </div>
              )}
            </div>
